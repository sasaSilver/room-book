...
class TimeRangeCustom(Keyboard):
    ...
    def get_blocked_timepoints(
        self,
        selected_time: datetime.time | None,
        daily_bookings: list[Booking]
    ) -> list[datetime.time]:
        blocked_timepoints = []
        ...
        for timepoint in self.timepoints:
            for booking in daily_bookings:
                ...
                if selected_time is None:
                    ...
                    (
                        booking_start_time < selected_time
                        and booking_end_time <= selected_time
                    )
                    or (
                        booking_start_time >= timepoint and booking_end_time > timepoint
                    )
                ):
                    blocked_timepoints.append(timepoint)
                    break
    ...
    def _get_keyboard_state(
        self,
        endpoint_time_selected: list[datetime.time],
        daily_bookings: list[Booking]
    ) -> tuple[dict[datetime.time, Booking], list[datetime.time], list[datetime.time]]:
        """
    ...
    def _get_filtered_timeslots(
        ...
        if selected_date == datetime.datetime.now().date():
            return list(
                filter(lambda x: datetime.datetime.now().time() <= x, self.timepoints)
            )
    ...
    async def _render_keyboard(
        ...
        (
            already_booked_timepoints,
            available_timepoints_to_select,
            blocked_timepoints,
        ) = self._get_keyboard_state(endpoint_time_selected, daily_bookings)
        ...
        for timepoint in timeslots:
            ...
            if already_selected:
                ...
                keyboard_builder.button(
                    text=timepoint.strftime("%H:%M"), callback_data=time_callback_data
                )
                ...
                if booking.username == manager.event.from_user.username:
                    ...
                    keyboard_builder.button(
                        text="🔴", url=f"https://t.me/{booking.username}"
                    )
            else:
                keyboard_builder.button(
                    text=" ",
